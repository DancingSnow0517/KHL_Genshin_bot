...
class Config:
    ...
    RECORD_URL = 'https://api-takumi.mihoyo.com/game_record/app/genshin/api/dailyNote?role_id={}&server={}'
...
async def req(method, url, max_retry: int = 2, params=None, data=None, json=None, headers=None, **kwargs):
    async with aiohttp.ClientSession() as session:
        for i in range(max_retry + 1):
            try:
                async with session.request(method, url, params=params, data=data, json=json, headers=headers,
                                           proxy='', **kwargs) as response:
